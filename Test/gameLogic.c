...
int runGame(int mode) {
	...
	if (FT_Init_FreeType(&ft) != 0) {
		fprintf(stderr, "ERROR: Couldn't initialize FreeType library\n");
		return 1;
	}

	if (FT_New_Face(ft, "font/DFCraftDou-W3-WING-RKSJ-H.ttf", 0, &face) != 0) {
	...
	/* Generate textures and vbos for font */
	glGenBuffers(1, &fontBuffer);
	glGenVertexArrays(1, &fontVertexAry);
	...
	fontShaderProgram = createFontShaderProgram();
	glBindAttribLocation(fontShaderProgram, 0, "in_Position");
	texUniform = glGetUniformLocation(fontShaderProgram, "tex");
	colorUniform = glGetUniformLocation(fontShaderProgram, "color");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glEnableVertexAttribArray(0);
		glBindVertexArray(fontVertexAry);
		...
		renderText("HeLLOWORLD", face, -0.5f, 0.0f, scaleX, scaleY);
		glDisable(GL_BLEND);
		*/
